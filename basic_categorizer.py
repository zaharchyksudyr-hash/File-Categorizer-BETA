# Наразі це тестовий режим ключові слова + embeddings. Існує лише у форматі коментарів та не є активним у зібраній ехе збірці.
# Як він працює:
# Створює ембеддинги для всіх основних категорій та підкатегорій (один раз при першому запуску).
# Нормалізує текст (нижній регістр, прибирає зайві пробіли).
# Порівнює текст із головними категоріями через cosine similarity та формує shortlist найбільш схожих категорій.
# Перевіряє ключові слова лише серед категорій із shortlist, щоб уточнити вибір.
# Вибирає головну категорію:
  # якщо ключових слів немає, але семантична схожість достатньо висока → використовує ембеддинги;
  # якщо знайдено ключові слова → віддає перевагу їм;
  # якщо впевненість низька → повертає Misc.
  # Аналогічно визначає підкатегорію всередині знайденої головної категорії.
  # Повертає результат у вигляді:
    # Категорія/Підкатегорія;
    # оцінку впевненості (similarity score);
    # інформацію про метод класифікації (чи використовувались ембеддинги, ключові слова або обидва).
